...
function getCustomerPortalTicketColumns() {
  return [
    { key: "name", label: __("Ticket ID"), type: "Data", width: "8rem" },
    { key: "subject", label: __("Subject"), type: "Data", width: "18rem" },
    { key: "status", label: __("Status"), type: "Select", width: "10rem" },
    {
      key: "custom_service_area",
      label: __("Service Area"),
      type: "Select",
      width: "12rem",
    },
    {
      key: "custom_equipment_ref",
      label: __("Equipment Ref"),
      type: "Data",
      width: "14rem",
    },
    {
...
function getCustomerPortalTicketRows() {
  return [
    "name",
    "subject",
    "status",
...
function shouldUseCustomerPortalTicketDefaultColumns() {
  return (
    options.value.isCustomerPortal && options.value.doctype === "HD Ticket"
  );
}
...
function updateColumns(obj) {
  ...
  const shouldUseNativeDefaultColumns =
    isDefault && !options.value.isCustomerPortal;

  columns.value = defaultParams.columns = shouldUseNativeDefaultColumns
    ? ""
    : _columns;

  defaultParams.rows = shouldUseNativeDefaultColumns ? "" : rows;
...
function reload(reset: boolean = false) {
  if (reset) {
    ...
    if (shouldUseCustomerPortalTicketDefaultColumns()) {
      defaultParams.columns = getCustomerPortalTicketColumns();
      defaultParams.rows = getCustomerPortalTicketRows();
    } else {
...
function handleViewChanges() {
  ...
  const shouldUseNativeDefaultColumns =
    defaultParams.is_default && !options.value.isCustomerPortal;

  defaultParams.filters = currentView.filters;
  defaultParams.order_by = currentView.order_by || "modified desc";
  defaultParams.columns = shouldUseNativeDefaultColumns
    ? ""
    : currentView.columns;
  defaultParams.rows = shouldUseNativeDefaultColumns ? "" : currentView.rows;

<template>
  <div v-if="showCustomerArticleSuggestions">
    <div
...
// Customer article suggestions are intentionally suppressed for the TELECTRO V1
// customer portal. The backend search endpoint is also overridden to return no
// results, because Redis/RediSearch-backed knowledge-base suggestions are out
// of scope for first production deployment.
const showCustomerArticleSuggestions = false;
...
watch(
  () => query,
  (query) => {
    if (!showCustomerArticleSuggestions) return;

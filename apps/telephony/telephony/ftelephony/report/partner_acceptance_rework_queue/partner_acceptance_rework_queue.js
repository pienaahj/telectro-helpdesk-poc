frappe.query_reports["Partner Acceptance Rework Queue"] = {
  ...
  formatter(value, row, column, data, default_formatter) {
    if (column.fieldname === "latest_partner_rework_note") {
      ...
      const marker = "Partner Acceptance Rework Required | Reason:";
      const cleanedText = text.startsWith(marker)
        ? text.slice(marker.length).trim()
        : text;

      const preview = cleanedText || text;

      const escapedPreview = frappe.utils.escape_html(preview);
      const escapedFullText = frappe.utils.escape_html(text);
      const escapedCleanedText = frappe.utils.escape_html(cleanedText);

      const message = JSON.stringify(`
        <div style="white-space: pre-wrap; line-height: 1.5;">
          <div style="font-weight: 600; margin-bottom: 8px;">Partner Acceptance Rework Reason</div>
          <div>${escapedCleanedText}</div>
        </div>
      `);
      ...
      return `
        <div class="partner-note-preview" title="${escapedFullText}">
          ...
          <a
            href="#"
            class="ml-2"
            onclick='frappe.msgprint({title: "Partner Acceptance Rework Reason", message: ${message}, wide: true}); return false;'
          >View full</a>
        </div>
      `;
    }
...
function inject_note_cell_css(report) {
  ...
  if (wrapper.find("#partner-rework-note-report-css").length) {
    return;
  }

  wrapper.append(`
    <style id="partner-rework-note-report-css">

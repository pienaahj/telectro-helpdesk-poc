frappe.query_reports["Partner Current Work"] = {
  filters: [],
  ...
  formatter(value, row, column, data, default_formatter) {
    if (column.fieldname === "latest_partner_note") {
      ...
      const escapedPreview = frappe.utils.escape_html(text);
      ...
      return `
        <div class="partner-current-work-note-preview" title="${escapedFullText}">
...
function inject_note_cell_css(report) {
  ...
  if (wrapper.find("#partner-current-work-note-report-css").length) {
    return;
  }

  wrapper.append(`
    <style id="partner-current-work-note-report-css">
      .partner-current-work-note-preview {
      ...
      .partner-current-work-note-preview a {
        white-space: nowrap !important;
      }
    </style>
  `);

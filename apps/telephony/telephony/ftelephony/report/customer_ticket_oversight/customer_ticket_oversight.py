    ...
    return [
        {
            ...
            "fieldname": "first_response_risk",
            "fieldtype": "Data",
            "width": 150,
        },
        {
            "label": "Time Left",
            "fieldname": "time_left_to_first_response",
            "fieldtype": "Data",
            "width": 130,
        },
        {
...
def get_data(filters):
    ...
    rows = frappe.db.sql(
        ...
        where
            ...
            and ifnull(t.custom_request_source, '') = 'Customer'
            and t.first_responded_on is null
    ...
    for row in rows:
        ...
        row["first_response_risk"] = _first_response_risk(row, now)
        row["time_left_to_first_response"] = _time_left_to_first_response(row, now)
        row["age"] = _age(row, now)

    return sorted(
        rows,
        key=lambda row: (
            _risk_sort(row.get("first_response_risk")),
            _response_by_sort(row.get("response_by")),
...
def _first_response_risk(row, now) -> str:
    ...
    response_by = get_datetime(response_by)
    seconds_left = time_diff_in_seconds(response_by, now)
    ...
    if response_by.date() == now.date():
        return "Due today"

    return "OK"


def _time_left_to_first_response(row, now) -> str:
    if row.get("first_responded_on"):
        return "Responded"
    ...
    response_by = get_datetime(response_by)
    seconds_left = int(time_diff_in_seconds(response_by, now))
...
def _risk_sort(risk: str) -> int:
    # Sort for operational chase priority:
    # first show tickets where the first-response SLA can still be saved.
    # Already-breached tickets still need follow-up, but they are no longer
    # first-response prevention work.
    order = {
        "Due < 15m": 10,
        "Due < 1h": 20,
        "Due today": 30,

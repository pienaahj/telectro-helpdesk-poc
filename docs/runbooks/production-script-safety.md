...
| `bin/release-runtime-validate.sh` | Local immutable runtime-image validator | Validates the completed candidate image locally before transfer: requires `linux/amd64`, requires the runtime `CI` environment variable to be unset, proves all four application directories and Python imports, verifies exact application versions, hashes a release-relevant file inside the image, and runs a repository-controlled semantic validator read-only inside an isolated disposable container. It retains validation logs, rejects CI-tainted runtimes, mismatched embedded content, and semantic failures, and emits aggregate success only after all applicable Phase 5 layers pass. It does not access or mutate production. |
| `bin/release-image-archive.sh` | Local immutable runtime-image archive builder and validator | Packages the already-validated candidate image with `docker save` and `gzip -1`, validates gzip integrity, records archive size and SHA-256, requires exactly one manifest entry for the immutable candidate tag, proves the embedded config blob against its declared digest for both supported Docker archive layouts, rejects archive or sidecar destination reuse, removes rejected partial archives, emits aggregate Phase 6 success only after all validation layers pass, and publishes a portable `<archive>.sha256` sidecar using the archive basename. It runs locally and does not access or mutate production. |
